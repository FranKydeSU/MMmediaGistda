...
describe('layerinfo epics', () => {
    ...
    it('layerInfoSetupLayersEpic', (done) => {
        ...
        testEpic(layerInfoSetupLayersEpic, 4, setControlProperty('layerinfo', 'enabled', true), actions => {
            ...
            expect(actions[2].layers[0].id).toBe(flatLayers[1].id);
            expect(actions[2].layers[0].name).toBe(flatLayers[1].name);
            expect(actions[2].layers[0].title).toBe(flatLayers[1].title);
            expect(actions[2].layers[0].description).toBe(flatLayers[1].description);
            expect(actions[2].layers[0].type).toBe('wmts');
            expect(actions[2].layers[0].layerObj).toEqual(flatLayers[1]);
            expect(actions[2].layers[0].selected).toBe(false);
            expect(actions[2].layers[0].syncStatus).toBe('none');
            expect(actions[2].layers[1].id).toBe(flatLayers[0].id);
            expect(actions[2].layers[1].name).toBe(flatLayers[0].name);
            expect(actions[2].layers[1].title).toBe(flatLayers[0].title);
            expect(actions[2].layers[1].description).toBe(flatLayers[0].description);
            expect(actions[2].layers[1].type).toBe('wms');
            expect(actions[2].layers[1].layerObj).toEqual(flatLayers[0]);
